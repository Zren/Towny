...
public class TownyUniverse extends TownyObject {
	...
	public TownyUniverse(Towny plugin) {
	    ...
	    Calendar result = new GregorianCalendar(
	    		tomorrow.get(Calendar.YEAR), tomorrow.get(Calendar.MONTH),
	    		tomorrow.get(Calendar.DATE), 0, 0);
	...
	public void setWarTime(boolean warTime) throws TownyException {
		if (this.warTime == warTime)
			throw new TownyException("War timer is already set to" + warTime);
		...
		if (this.warTime) {
			...
			for (Long delay : TimeMgmt.getCountdownDelays(36000000, TimeMgmt.defaultCountdownDelays)) {
				warTimer.schedule(
					new ServerBroadCastTimerTask(
						plugin,
						String.format("War starts in %s", TimeMgmt.formatCountdownTime(delay))),
					delay);
			}
			warTimer.schedule(new StartWarTimerTask(this), settings.getWarTimeWarningDelay());
